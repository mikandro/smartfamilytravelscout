...
async def preferences_page(request: Request, db: AsyncSession = Depends(get_async_session)):
...
@router.get("/stats")
async def stats_page(request: Request, db: AsyncSession = Depends(get_async_session)):
